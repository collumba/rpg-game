...
function setupMainButtons() {
  ...
  if (!window.sceneManager) {
    console.error(
      "SceneManager não está disponível! Tentando novamente em 500ms..."
    );
    setTimeout(setupMainButtons, 500);
    return;
  }

  // Botão de iniciar jogo (tela inicial -> seleção de personagens)
  const startButton = document.getElementById("startButton");
  if (startButton) {
    console.log("Adicionando evento de clique ao botão de iniciar jogo");
  ...
  // Botão de confirmar time (seleção de personagens -> batalha)
  const confirmTeam = document.getElementById("confirmTeam");
  if (confirmTeam) {
    console.log("Adicionando evento de clique ao botão de confirmar time");

    // Remove listeners anteriores para evitar duplicação
    confirmTeam.replaceWith(confirmTeam.cloneNode(true));
    const newConfirmTeam = document.getElementById("confirmTeam");

    newConfirmTeam.addEventListener("click", () => {
      console.log("Botão de confirmar time clicado");
      if (selectedCharacters.length > 0) {
        window.sceneManager.transitionTo("battle", initializeBattle);
      } else {
        alert("Selecione pelo menos um personagem para seu time!");
      }
    });
  } else {
    console.error("Botão de confirmar time não encontrado!");
  }

  // Botão de reiniciar jogo (game over -> tela inicial)
  const restartButton = document.getElementById("restartButton");
  if (restartButton) {
    console.log("Adicionando evento de clique ao botão de reiniciar jogo");
    ...
    newRestartButton.addEventListener("click", () => {
      console.log("Botão de reiniciar jogo clicado");
      resetGame();
      window.sceneManager.transitionTo("title");
    });
  } else {
    console.error("Botão de reiniciar jogo não encontrado!");
  }

  console.log("Botões principais configurados com sucesso!");
}
...
function resetGame() {
  ...
  document.querySelector(".enemy-team").innerHTML = "";

  // Reseta a seleção de personagens
  document.querySelectorAll(".character-card").forEach((card) => {
    card.classList.remove("selected");
  });

  // Limpa os slots do time
  updateTeamDisplay();

  // Desabilita o botão de confirmar time
  document.getElementById("confirmTeam").disabled = true;
}

// Event Listeners
document.addEventListener("DOMContentLoaded", () => {
  // Inicializa os slots do time
  initializeTeamSlots();

  // Inicializa os cards de personagens
...
const characters = {
  melee: [
    ...
    },
    {
      name: "Ladino",
      emoji: "🏹🗡️",
      description: "Rápido, furtivo, especialista em golpes críticos e evasão.",
...
function createCharacterCard(character) {
  ...
  if (!character.stats.maxMana) character.stats.maxMana = character.stats.mana;
    ...
    if (selectedCharacters[index]) {
      ...
      slot.innerHTML = `
        <img src="assets/characters/${
          char.avatar
        }" class="character-profile-image" alt="${char.name}">
...
function initializeCharacterCards() {
  ...
  allCharacters.forEach((char) => {
    ...
    card.addEventListener("click", () => toggleCharacterSelection(card, char));
...
function startGame() {
  ...
  if (selectedCharacters.length > 0) {
    console.log("Iniciando jogo com", selectedCharacters.length, "personagens");

    if (window.sceneManager) {
      window.sceneManager.transitionTo("battle", initializeBattle);
    } else {
      console.error("SceneManager não está disponível!");
      alert("Erro ao iniciar o jogo. Por favor, recarregue a página.");
    }
  } else {
    console.warn("Tentativa de iniciar jogo sem personagens selecionados");
    alert("Selecione pelo menos um personagem para seu time!");
  }
}
...
const bosses = [
  {
    ...
    description: "Um guerreiro espectral que traz o apocalipse em sua lâmina.",
    stats: {
  ...
  },
  {
    ...
    description: "Um mestre da magia negra que comanda um exército de mortos.",
    stats: {
      hp: getRandomHP(20, 30), // HP aleatório entre 30 e 45
      attack: 5,
    },
...
function setupActionButtons() {
  ...
  actionButtons.forEach((button) => {
    ...
    button.parentNode.replaceChild(newButton, button); // Substitui o botão antigo pelo novo
  });

  // Re-obter os botões após a substituição
  const updatedActionButtons = document.querySelectorAll(".action-button");

  updatedActionButtons.forEach((button) => {
...
function performSkill(character) {
  ...
  const targetingStarted = setupTargetSelection(character, (selectedTarget) => {
    if (
      selectedTarget === null &&
...
function executeActiveAbility(character, target) {
  ...
  switch (character.name) {
    ...
    case "Paladino":
      if (character.stats.mana >= manaCost && target && target.stats.hp <= 0) {
...
function endGame(victory) {
  ...
  document.getElementById("gameResultMessage").textContent = resultMessage;

  // Transição para a cena de fim de jogo
